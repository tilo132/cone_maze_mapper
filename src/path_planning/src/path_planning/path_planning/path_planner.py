...
from numpy import asfortranarray, vstack, array, flipud
...
class PathPlanner(Node):
    ...
    def __current_position_listener_callback(self, current_position: CurrentPosition):
        ...
        """
        self.vec_pos = array([current_position.vehicle_position_x, current_position.vehicle_position_y])
        if self.mock_current_position != True:
            logging.debug(f'Set new Position: x:{current_position.vehicle_position_x} y:{current_position.vehicle_position_y}')
    ...
    def __cone_listener_callback(self, next_cone: Cone):
        ...
        if self.mode == Mode.EXPLORATION:
            ...
            if planned_path:
                ...
                dist_to_first_point = norm(self.vec_pos - array(planned_path[0]))
                dist_to_last_point = norm(self.vec_pos - array(planned_path[-1]))

                first_is_closer = dist_to_first_point < dist_to_last_point

                logging.info("first closer: {}".format(first_is_closer))

                points = flipud(vstack((planned_path, array([self.vec_pos]))))
                ...
                for i in range(len(points)):
                    #s = i / (len(points)-1)
                    #position = curve.evaluate(s)
                    #direction = curve.evaluate_hodograph(s)
                    ...
                    pose.pose.position.x = position.item(0)
                    pose.pose.position.y = position.item(1)
                    pose.pose.position.z = 0.0
                    pose.pose.orientation.x = 0.0 # quaternion math
                    pose.pose.orientation.y = 0.0 # quaternion math
                    pose.pose.orientation.z = sin(angle/2) # quaternion math
                    pose.pose.orientation.w = cos(angle/2) # quaternion math
                    path.poses.append(pose)
                self.planned_path_publisher.publish(path)
